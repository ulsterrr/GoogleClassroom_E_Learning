  ...
  <body>
    <div class="mt-5 col-10 col-md-7 col-lg-3 text-center mx-auto">
      ...
      <form action="{{ route('xl-cap-nhat-tai-khoan') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
          <input class="form-control py-3" name="username" value="{{ auth()->user()->username }}"  placeholder="Username" readonly/>
        </div>
        <div class="mb-3">
          <input
            type="password"
        ...
        <div class="mb-3">
          <input class="form-control py-3" name="hoten" value="{{ auth()->user()->hoten }}" placeholder="Full Name" />
        </div>
        <div class="mb-3">
          <input type="email" name="email" value="{{ auth()->user()->email }}" class="form-control py-3" placeholder="Email" />
        </div>
        <div class="mb-3">
          <input
            name="sdt"
